...
#     input:
#         expand("outputs/{{scenario}}/{pipeline}.parquet", pipeline=WORKFLOWS)[0],
#     output:
#         expand("outputs/{{scenario}}/sphering/exploration/{pipeline}_reg{{reg}}.parquet", pipeline=WORKFLOWS)[0],
#         expand("outputs/{{scenario}}/sphering/exploration/{pipeline}_reg{{reg}}.npz", pipeline=WORKFLOWS)[0],
#     params:
#         method=config["sphering_method"],
#         reg=lambda wc: round(float(wc.reg), 5),
